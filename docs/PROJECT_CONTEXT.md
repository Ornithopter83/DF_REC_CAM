...
- `supabase/migrations`의 001~013이 원격 프로젝트에 적용되어 있다.
...
- 신규 등록에서 새로 생성되는 카메라의 표시명과 NAS 폴더 세그먼트는 관리자가 입력한 등록 PC명을 기준으로 한다. 이미 카메라와 녹화 카탈로그가 있는 재등록은 기존 이름과 NAS 경로를 유지한다.
